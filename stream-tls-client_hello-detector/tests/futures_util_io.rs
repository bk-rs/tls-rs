...
mod futures_util_io_tests {
    ...
    // ref https://github.com/vkill/rust-io-peek/blob/master/futures-util-io-peek/tests/async_io_async.rs
    ...
    #[test]
    fn async_io_async_tcp_stream() -> io::Result<()> {
        block_on(async {
            ...
            executor
                .spawn(async move {
                    ...
                    let client_hello_payload =
                        detector.detect_async(&mut tcp_stream_s).await.expect(
                            "detector
                        .detect_async failed",
                        );
